...
def send_csv_data():
  ...
  mail_text = \
    'simpway管理者さま\n\n'\
    + 'この度はSimpwayをご利用頂き誠にありがとうございます。\n'\
    + '下記のリンクから本日分の予約データのダウンロードをお願い致します。\n'\
    + 'http://partner.simpway.to/download?m=olix'
  to_emails = [
    ('simpway-notice@pfr.technology', 'simpway運営'),
    ('itonaga0322@gmail.com', 'Itonaga Yuma')
  ]
  message = Mail(
        from_email='noreply@simpway.to',
        to_emails=to_emails,
        subject='Simpway本日のデータ（' + str(datetime.date.today()) + '予約分）',
        plain_text_content=mail_text
        )
